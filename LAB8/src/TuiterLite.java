...
public class TuiterLite<T>{
    ...
    public Tuite tuitarAlgo(Usuario usuario, String texto) throws UsuarioDesconhecidoException {
        ...
        try{
            if(texto == null || usuario == null){throw new IllegalArgumentException();}
            if(texto.length() == 0){throw new TamanhoMinimoException();}
            if(!this.usuarioByEmail.containsKey(usuario.getEmail())){throw new UsuarioDesconhecidoException();}
            if (texto.length() > TAMANHO_MAXIMO_TUITES){throw new TamanhoMaximoExcedidoException();}
        }
        catch (IllegalArgumentException e){
            System.out.println("Texto ou usuário não podem ser null");
            return null;
        }
        catch (TamanhoMinimoException e) {
            System.out.println("Tamanho mínimo não atendido!");
            return null;
        }
